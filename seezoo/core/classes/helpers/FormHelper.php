...
class SZ_FormHelper
{
	...
	/**
	 * get value
	 * ## this method returns Validation::run executed after only! ##
	 * 
	 * @access public
	 * @param  string $key
	 * @param  bool   $escape
	 * @return string
	 */
	public function value($key, $escape = TRUE)
	{
		if ( ! is_array($this->_validatedValues) )
		{
			if ( ! SeezooFactory::exists('libraries', 'validation') )
			{
				return '';
			}
			$Validation = Seezoo::$Importer->library('Validation');
			$this->_validatedValues = $Validation->getValidatedValues();
		}
		
		if ( ! isset($this->_validatedValues[$key]) )
		{
			return '';
		}
		
		return ( $escape )
		        ? prep_str($this->_validatedValues[$key])
		        : $this->_validatedValues[$key];
	}
